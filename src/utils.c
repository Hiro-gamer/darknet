...
int find_int_arg(int argc, char **argv, char *arg, int def)
{
    ...
    for(i = 0; i < argc-1; ++i){
        if(!argv[i]) continue;
        if(0==strcmp(argv[i], arg)){
            //ì¸óÕÇ™NULLÇæÇ¡ÇΩÇÁÅAé©ìÆÇ≈1Ç…Ç∑ÇÈ
...
void check_warning(int class_id_counter[], int default_class_num)
{
    ...
    printf("-------------åxçê-------------\n");
    /*
    // óßÇ¡ÇƒÇÈêlÇ™ëΩÇ©Ç¡ÇΩÇÁÅAåxçê
    if (class_id_counter[1] > 6) printf("ëÚéRÇÃêlÇ™óßÇ¡ÇƒÇ¢Ç‹Ç∑ÅI\n");
    // óßÇ¡ÇƒÇ¢ÇƒÅAÇ¬ÇËävÇéùÇ¡ÇƒÇ¢Ç»Ç¢êlÇ™5êlà»è„Ç¢ÇΩÇÁåxçê
    if (abs(class_id_counter[0] - class_id_counter[1]) >= 3) printf("Ç¬ÇËävÇÃéùÇ¡ÇƒÇ¢ÇÈêlÇ™è≠Ç»Ç¢Ç≈Ç∑ÅI\n");
    */
    printf("------------------------------\n");
    return;
}